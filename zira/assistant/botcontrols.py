...
async def bot_help(event):
    await event.reply(
        """-قائمه اوامر البوت**
**⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆**
**📑 اولاً الاوامر الإدارية الخاصه بالبوت المسـاعد الخـاص بـك :**
**ملاحـظــه :**
تعمـل هذه الأوامـر فقـط بخـاص البـوت المسـاعـد

/info <بالـرد ع رسـالة الشخـص>
**لـ بجلب معلومات المرسـل حتى لو مغلـق الخصوصيـه في حسابه على عكس بوتـات التواصـل العاديـه**
    ...
    for user in users:
        try:
            ...
            if count % 5 == 0:
                try:
                    prog_ = (
                        "** جـاري الاذاعه لمستخدمين البـوت ...**\n\n"
                        + progress_str(
                            total=bot_users_count,
                            current=count + len(blocked_users),
                        )
                        + f"\n\n• ✔ **تم بنجـاح** :  `{count}`\n"
                        + f"• × **خطـأ باذاعـة** :  `{len(blocked_users)}`"
                    )
